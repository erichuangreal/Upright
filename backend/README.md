...
Overview:
- Express + WebSocket server that accepts IMU samples and events from one or two Arduino devices.
...
- `server.js`: Main backend. Endpoints:
	- `POST /imu` (source 1) and `POST /imu2` (source 2) — accept samples (must include numeric `pitch`) or events (`event` string).
