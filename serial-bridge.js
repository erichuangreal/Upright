...
// and POSTs each JSON object to your Node backend (/imu by default).
//
// Usage (PowerShell):
//   & "C:\Program Files\nodejs\node.exe" serial-bridge.js COM5 115200 localhost 8080 /imu
//
// Notes:
// - Close Arduino Serial Monitor/Plotter first, or COM5 will be "busy".
...
// Args
const PORT_NAME = process.argv[2]; // e.g. COM5
...
// Optional: show once per second that we're alive
let sent = 0;
setInterval(() => {
...
parser.on("data", (line) => {
    ...
    // Ignore non-JSON noise (in case Arduino prints other text)
    if (!s.startsWith("{") || !s.endsWith("}")) return;

    try {
        const obj = JSON.parse(s);

        // Tiny sanity check: require pitch
        if (typeof obj.pitch !== "number") return;
